...
def model_log_in_web(driver,url, login_info):
    ...
    if (login_info["captcha"]!="") :
        image = login_info["img"]
        captcha = login_info["captcha"]
        capt = get_image_captcha(driver,image)
        if captcha["type"] == "id":
            driver.find_element_by_id(captcha["name"]).send_keys(capt)
        elif captcha["type"] == "class":
            driver.find_element_by_class_name(captcha["name"]).send_keys(capt)
        elif captcha["type"] == "xpath":
            driver.find_element_by_xpath(captcha["name"]).send_keys(capt)
        #driver.find_element_by_id(login_info["captcha"]).send_keys(capt)
    ######################################################################
    button = login_info["button"]
    if button["type"] == "id":
...
############################zl 粘贴 手动输入验证码####################################
def get_position(driver,image):
    """
    获取验证码位置
    :return: 验证码位置元组
    """
    if image["type"] == "id":
        img = WebDriverWait(driver, 10, 0.5).until(EC.presence_of_element_located((By.ID, image["name"])))
    elif image["type"] == "class":
        img = WebDriverWait(driver, 10, 0.5).until(EC.presence_of_element_located((By.CLASS_NAME, image["name"])))
    elif image["type"] == "xpath":
        img = WebDriverWait(driver, 10, 0.5).until(EC.presence_of_element_located((By.XPATH, image["name"])))



    #img = WebDriverWait(driver, 10, 0.5).until(EC.presence_of_element_located((By.ID, 'randimg')))
    time.sleep(2)
...
def get_image_captcha(driver,image, name='captcha.png'):
    """
    获取验证码图片
    :return: 验证码
    """
    top, bottom, left, right = get_position(driver,image)
